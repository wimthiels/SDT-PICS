...
df_prm= pd.DataFrame({'values':list(prm.values())},index=list(prm.keys()))
p_csv_param = prm['output_folder'] / "param.csv"
df_prm.to_csv(p_csv_param,header=False,index=True)
...
for R_module in prm['l_R_modules']:
	...
	args = [str(p_csv_param)]   #pass on csv file with all parameters
